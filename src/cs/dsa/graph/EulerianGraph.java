 ...
 * 
 * 
 * Given a directed graph, check whether it has an Eulerian path or not. An Eulerian path 
 * (or Eulerian trail) is a path in a graph that visits every edge exactly once.

	The following graph has an Eulerian path since it is possible to construct a path that 
	visits each edge exactly once. The Eulerian path is 0 —> 1 —> 2 —> 3 —> 0 —> 5 —> 4 —> 3 —> 1 —> 4.
